...
use crate::utils::{cache, get_package, handle_error};
...
pub async fn updater(packages: Vec<String>) {
  ...
  for pkg in packages.iter() {
    ...
    let exists = check_cache(package_name.clone(), latest_version, file_type);
  ...
  for handle in handles {
    handle
      .join()
      .unwrap_or_else(|_| handle_error_and_exit("Failed to join handles".to_string()));
  }
