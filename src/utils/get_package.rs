...
pub async fn get_package(package_name: &str) -> Package {
    ...
    let response = get(format!("https://storage.googleapis.com/novus_bucket/packages/{}.json", package_name)).await.unwrap_or_else(|e| handle_error_and_exit(e.to_string()));
    file_contents = response.text().await.unwrap_or_else(|e| handle_error_and_exit(format!("{} get_package.rs:36", e.to_string())));
...
pub async fn get_packages() -> String {
    let mut file_contents = String::new();
    let response = get("https://storage.googleapis.com/novus_bucket/package-list.json").await.unwrap_or_else(|e| handle_error_and_exit(e.to_string()));
    file_contents = response.text().await.unwrap_or_else(|e| handle_error_and_exit(format!("{} get_package.rs:36", e.to_string())));
