...
pub async fn get_package(package_name: &str) -> Package {
    ...
    let response = get(format!("https://storage.googleapis.com/novus_bucket/packages/{}.json?a={:?}", package_name, std::time::UNIX_EPOCH.elapsed().unwrap())).await.unwrap_or_else(|e| handle_error_and_exit(e.to_string()));
